...
export default function SafetySummary({
  ...
  const safeVenueRiskFactors = venueRiskFactors ?? [];
  const safeEmergencyExits = emergencyExits ?? [];

  if (!riskSummary && safeVenueRiskFactors.length === 0 && safeEmergencyExits.length === 0) {
    return null;
  }
      ...
      <CardContent className="pt-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            {safeEmergencyExits.length > 0 && (
              <p className="text-xs text-sage-600 mt-3">
                <span className="font-semibold">Emergency exits:</span> {safeEmergencyExits.slice(0, 2).join(" · ")}
              </p>
            )}
          </div>
        </div>
